...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">@lang('site.admins') <small>{{$users->total()}}</small>  </h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            @if(auth()->user()->hasPermission('create_users'))
                                <a class="btn bg-gradient-success btn-sm" href="{{route('dashboard.admins.create')}}"><i class="fa fa-plus px-1"></i>@lang('site.add')</a>
                            @else
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(auth()->user()->hasPermission('update_users'))
                                            <a class="btn btn-info btn-sm" href="{{route('dashboard.admins.edit',$user->id)}}"><i class="fa fa-edit px-1"></i>@lang('site.edit')</a>
                                        @else
                                            <button disabled class="btn btn-info btn-sm">@lang('site.edit')</button>
                                        @endif
                                        @if(auth()->user()->hasPermission('delete_users'))
                                            <form class="d-inline-block" action="{{route('dashboard.admins.destroy',$user->id)}}" method="post">
                                                {{csrf_field()}}
                                                {{method_field('delete')}}
